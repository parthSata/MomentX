    ...
    return (
        <div className="h-full w-full flex items-center justify-center bg-black relative">
            ...
            <div className="relative h-full w-full max-w-105 mx-auto bg-black md:rounded-xl overflow-hidden shadow-2xl" onDoubleClick={handleDoubleTap}>
                ...
                <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={(e) => { e.stopPropagation(); onToggleMute(); }} className="absolute top-4 right-4 z-30 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center hover:bg-black/60 transition-colors">
                    {muted ? <VolumeX className="w-4 h-4 text-white" /> : <Volume2 className="w-4 h-4 text-white" />}
                </motion.button>

                <AnimatePresence>
                    {showHeart && isActive && (
                        <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} className="absolute inset-0 flex items-center justify-center pointer-events-none z-50">
                ...
                <div className="absolute right-3 bottom-24 md:bottom-20 flex flex-col items-center gap-4 z-20">
                    ...
                    <button className="flex flex-col items-center gap-1 active:scale-90 transition-transform">
                        ...
                        <span className="text-white text-xs font-semibold">{reel.sharesCount}</span>
                    </button>
