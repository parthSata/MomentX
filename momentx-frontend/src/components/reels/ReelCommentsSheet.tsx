...
export function ReelCommentsSheet({ isOpen, onClose, postId, commentCount, onCommentAdded }: ReelCommentsSheetProps) {
    ...
    // ... (toggleLike, handleDelete, other helpers remain the same as previous) ...
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="absolute bottom-0 left-0 right-0 z-50 bg-neutral-900 rounded-t-xl overflow-hidden flex flex-col h-[70%]"
                    ...
                    >
                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-neutral-900">
                            <div className="flex flex-col">
                                ...
                                {/* Use the Passed Comment Count prop as base, or length if loaded */}
                        ...
                        <div className="p-3 border-t border-white/10 bg-black/90">
                            {replyingTo && (
                                <div className="flex items-center justify-between mb-2 px-3 py-1.5 bg-white/10 rounded-md">
                                    ...
                                    <button onClick={() => setReplyingTo(null)}><X className="w-3 h-3 text-white/70" /></button>
                                </div>
                            )}
                            <div className="flex items-center gap-3">
                                <img src={currentUser?.profilePic || "https://github.com/shadcn.png"} alt="Avatar" className="w-8 h-8 rounded-full object-cover shrink-0 border border-white/10" />
                                <div className="flex-1 relative">
                                    <input
                                        ...
                                        className="w-full bg-white/10 rounded-full pl-4 pr-10 py-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500/50"
                                        onKeyDown={(e) => e.key === 'Enter' && !sending && handleSend()}
                                        disabled={sending}
                                    />
                                    <button onClick={handleSend} disabled={!newComment.trim() || sending} className={cn("absolute right-1 top-1/2 -translate-y-1/2 p-1.5 rounded-full transition-colors", newComment.trim() ? "text-blue-400 hover:bg-white/10" : "text-white/20")}>
    ...
    return (
        <div className="space-y-3">
            <div className="flex gap-3 group">
                <img src={comment.user.profilePic || "https://github.com/shadcn.png"} alt="User" className="w-8 h-8 rounded-full object-cover shrink-0" />
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                        <span className="text-white text-xs font-semibold">{comment.user.username}</span>
                        {comment.user.isVerified && <BadgeCheck className="w-3 h-3 text-blue-400 fill-blue-400" />}
                        <span className="text-white/40 text-[10px]">{formatDistanceToNow(new Date(comment.createdAt), { addSuffix: true })}</span>
                    </div>
                    <p className="text-white text-xs leading-relaxed wrap-break-word">{comment.text}</p>
                    <div className="flex items-center gap-4 mt-1.5">
                        <button onClick={() => onReply(comment)} className="text-white/50 text-[10px] font-semibold hover:text-white transition-colors">Reply</button>
                        {comment.likes.length > 0 && <span className="text-white/40 text-[10px]">{comment.likes.length} likes</span>}
                        {isOwner && <button onClick={() => onDelete(comment._id)} className="text-white/30 hover:text-red-400 transition-colors"><Trash2 className="w-3 h-3" /></button>}
                    </div>
                    {comment.replies && comment.replies.length > 0 && (
                        <button onClick={() => toggleReplies(comment._id)} className="flex items-center gap-2 mt-2 text-white/50 text-[10px] hover:text-white transition-colors">
                            <div className="w-6 h-px bg-white/20" /> {expandedReplies[comment._id] ? "Hide replies" : `View ${comment.replies.length} replies`}
                        </button>
                    )}
                </div>
                <button onClick={() => onLike(comment._id)} className="shrink-0 p-1 self-start"><Heart className={cn("w-3.5 h-3.5 transition-colors", isLiked ? "text-red-500 fill-red-500" : "text-white/30 hover:text-white/60")} /></button>
            </div>
            <AnimatePresence>
                {expandedReplies[comment._id] && comment.replies && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="ml-11 space-y-3">
                        {comment.replies.map((reply: any) => (
                            <CommentItem key={reply._id} comment={reply} currentUser={currentUser} onLike={onLike} onReply={onReply} onDelete={onDelete} expandedReplies={expandedReplies} toggleReplies={toggleReplies} />
                        ))}
                    </motion.div>
                )}
            </AnimatePresence>
