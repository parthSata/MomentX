...
interface CreateGroupDialogProps {
    isOpen: boolean;
    onClose: () => void;
    // Updated type to match dynamic User model
...
export function CreateGroupDialog({ isOpen, onClose, onCreated }: CreateGroupDialogProps) {
    ...
    const [selectedUsers, setSelectedUsers] = useState<any[]>([]); // Store full objects for UI
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            setLoading(true);
            try {
                // ✅ Ensure the query parameter matches your backend route: query('username')
                ...
                // ✅ Check both .data and .message. 
                // Most of your controllers use ApiResponse which puts data in .data
                let users = [];
    ...
    const handleCreate = () => {
        ...
        // Pass IDs to the parent for the API call
        onCreated({
            name: groupName,
                            ...
                            {step === 1 ? (
                                        ...
                                        {availableUsers.map((user, i) => {
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    onClick={() => toggleUser(user)}
                                                    className={`flex items-center gap-3 p-3 rounded-2xl cursor-pointer transition-all ${selected ? "bg-primary/10 ring-1 ring-primary/30" : "hover:bg-muted/50"
                                                        }`}
                                                >
                                                    <div className="relative">
                                ...
                                >
                                    <div className="flex-1 flex flex-col items-center pt-8 px-6">
                                        ...
                                        >
                                            {/* ✅ Canonical class fix p-[3px] -> p-0.75 */}
                                            <div className="w-24 h-24 rounded-full bg-gradient-primary p-0.75 animate-glow">
